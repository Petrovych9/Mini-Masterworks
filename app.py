...
app = Flask(
    __name__)  # explanation:  When a Python script is executed directly, __name__ is set to '__main__'. When the script is imported as a module, __name__ is set to the module's name.
...
def saveData():
    if request.method == "POST":
        name, author, pages, booktype, today = utils.collectData()
        # format information to df
        data = utils.createDf(name, author, pages, booktype, today)
...
def getLocationByIP():
    if request.method == "POST":
        ip = request.form['ipp']        # 24.48.0.1 for example
